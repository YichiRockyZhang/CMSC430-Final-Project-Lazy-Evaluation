...
(define (compile-prim3 p e1 e2 e3 c)
  (seq (compile-e e1 c #f)
       ...
       (compile-e e2 (pad-env c) #f)
       (Push rax)
       (compile-e e3 (pad-env (pad-env c)) #f)
       (compile-op3 p)))
...
(define (compile-let x e1 e2 c t?)
  ...
  (seq 
        ;;; (compile-e e1 c #f)
       (Push 0)
       (compile-e e2 (cons (cons x e1) c) t?)
      (Add rsp 8)))
...
(define (compile-app-nontail e es c)
  (let ((r (gensym 'ret))
    ...
    (seq (Lea rax r)
         (Push rax)
         (compile-es (cons e es) (pad-env c))         
...
(define (compile-match e ps es c t?)
  (let ((done (gensym)))
    (seq (compile-e e c #f)
         ...
         (compile-match-clauses ps es (pad-env c) done t?)
    ...
    [(PLit l)
     (let ((fail (gensym)))
       (list (seq (Cmp rax (imm->bits l))
             ...
             cm))]
    [(PAnd p1 p2)
     (match (compile-pattern p1 (pad-env cm) next)
       [(list i1 f1 cm1)
