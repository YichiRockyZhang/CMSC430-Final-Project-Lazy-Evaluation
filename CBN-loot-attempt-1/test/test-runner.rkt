...
(define (test-runner run)
  ...
  (check-equal? (run '(let ((x 7)) (let ((a (add1 x))) a))) 8)
  (check-equal? (run '(let ((x (zero? #t))) 42)) 42)
  ...
  (check-equal? (run '(let ((x (cons 1 2)))
                        (let ((y (box 3)))
                          (unbox y))))
                3)
  (check-equal? (run '(let ((x (cons 1 (cons 2 '())) ))
    (let (( y (cons (zero? #t) x) ))
        (car (cdr y))))) #t)
  ...
  (check-equal? (run '((match 8 [8 (lambda (x) x)]) 12))
                12)
   ;; CNB-Loot examples
  (check-equal? (run '(let ((x (zero? #t))) x)) 'err) ;; same in normal Loot
  (check-equal? (run '(let ((x (zero? 0))) x)) #t) ;; same in normal Loot
  ...
  (check-equal? (run '(let ((x (zero? #t))) 42)) 42) ;; 'err in normal Loot
  (check-equal? (run '(let ((x (cons 1 (cons 2 '())) )) (let (( y (cons (zero? #t) x) )) (car (cdr y)))) ) 1) ;; 'err in normal Loot
   ;; 'err in normal Loot
  (check-equal? (run 
      '(define (k x y) x)
      '(k 42 (zero? #t))
  ...
  (check-equal? (run  ; SUPPOSED TO FAIL
      ...
      ;;; '(let ((evens (inf-seq 0 (lambda (x) (add1 (add1 x)))) ))
