#lang racket
(require "ast.rkt")
(provide interp-prim1 interp-prim2 interp-prim3 force)

;; Thunk -> Expr
;;; (define (force d) (match d [(Thunk e) (e)] [v v]))
(define (force d) (d))
...
(define (interp-prim1 p1 v)
  (match (list p1 v)
    ...
    [(list 'box v)                        (box v)]
    [(list 'unbox (? box?))               (force (unbox v))] ; CHANGED
    [(list 'car (? pair?))                (force (car v))] ; CHANGED
    [(list 'cdr (? pair?))                (force (cdr v))] ; CHANGED
