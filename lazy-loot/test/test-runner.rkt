...
(define (test-runner run)
  ...
  (check-equal? (run '(cons 1 2)) (cons 1 2)) ; SUPPOSED TO FAIL
  (check-equal? (run '(unbox (box 1))) 1) ; SUPPOSED TO FAIL
  (check-equal? (run '(car (cons 1 2))) 1)
  (check-equal? (run '(cdr (cons 1 2))) 2)
  (check-equal? (run '(cons 1 '())) (list 1)) ; SUPPOSED TO FAIL
  ...
  (check-equal? (run '(let ((x (cons 1 2))) (eq? x x))) #t) ; SUPPOSED TO FAIL
  ...
  (check-equal? (run '(let ((x (make-vector 3 5))) ; SUPPOSED TO FAIL
  ...
  (check-equal? (run
                 ...
                 '(even? 101))
                #f)
  (check-equal? (run ; SUPPOSED TO FAIL
                 '(define (map-add1 xs)
                    (if (empty? xs)
                        '()
                        (cons (add1 (car xs))
  ...
  (check-equal? (run '(let ((f (λ (x y) y))) (f 5 7)))
                7) 
  (check-equal? (run '((let ((x 1)) ; SUPPOSED TO FAIL
                         (let ((y 2))
  ...
  (check-equal? (run '(define (tri n)
   ...
   ;; Lazy-Loot examples
  (check-equal? (run '(let ((x (zero? #t))) x)) 'err) ;; same in strict Loot
  (check-equal? (run '(let ((x (zero? 0))) x)) #t) ;; same in strict Loot

   ;; same in strict Loot
  (check-equal? (run 
        '((let((f (lambda () 42) ))
        f))
      ) 42) ;; if one is not careful during implementation this could easily
               ;; attempt to evaluate (42) instead of (f)

  (check-equal? (run '(let ((x (zero? #t))) 42)) 42) ;; 'err in strict Loot
   ;; 'err in strict Loot
  (check-equal? (run 
      '(define (k x y) x)
      '(k 42 (zero? #t))
  ...
  (check-equal? (run  ; SUPPOSED TO FAIL
      '(define (inf-seq a0 f) (cons a0 (inf-seq (f a0) f)))
      '(define (inf-seq2 a0 a1 f) (cons a0 (inf-seq2 a1 (f a0 a1) f)))
      '(define (nth seq n)
         (if (zero? (sub1 n))
            (car seq)
            (nth (cdr seq) (sub1 n))))
      ;;; '(let ((naturals (cons 1 (cons 2 '())) ))
      ;;; '(let ((naturals (inf-seq 1 (lambda (x) (add1 x))) ))
      '(let ((fib (inf-seq2 0 1 (lambda (x1 x2) (+ x1 x2))) ))
         (nth fib 10))
  ) 34)
)
