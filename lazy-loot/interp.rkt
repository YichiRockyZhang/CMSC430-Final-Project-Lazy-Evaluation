...
(define (make-thunk e r ds)
  (Delay (interp-env (Lam (gensym 'thunk) '() e) r ds))
)
...
(define (interp-env e r ds)
  (match e
    ...
    [(Prim1 p e)
     (match p
       [(or 'box) (let ((ethunk (make-thunk e r ds)) )
    ...
    [(Prim2 p e1 e2)
     (match p
       ['cons (let ((e1thunk (make-thunk e1 r ds)) (e2thunk (make-thunk e2 r ds)))
    ...
    ;;;  failed implementation because lambda in racket does not capture the env for later (lazy evaluation)
     (let (( e1thunk (make-thunk e1 r ds) )) 
...
(define (interp-var x r ds)
  (match (lookup r x)
    ['err (match (defns-lookup ds x)
            ...
            [#f 'err])]
    [(Delay e) (e)] ; Forcing the evaluation of a thunk
    ;;; [(Lam _ '() e) (App e '())] ; Forcing the evaluation of a thunk
    [v v])) ; Normal variable
...
(define (interp-env* es r ds)
  (match es
    ['() '()]
    [(cons e es)
     (match (Delay (interp-env (Lam (gensym 'thunk) '() e) r ds)) ; (interp-env e r ds)
       ['err 'err]
